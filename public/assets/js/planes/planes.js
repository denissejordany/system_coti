...
// ==========================================
// MODAL CONTROL DE BOOTSTRAP 5 (ACTUALIZADO)
// ==========================================
let bootstrapPlanModal = null;

function abrirModalPlan() {
    // 1. PRIMERO: Preparamos y limpiamos el HTML internamente de forma estable
    const contenedorRedes = document.getElementById("contenedor-redes");
    const contenedorPrecios = document.getElementById("contenedor-precios");

    if (contenedorRedes) contenedorRedes.innerHTML = "";
    if (contenedorPrecios) contenedorPrecios.innerHTML = "";
    
    // Inyectamos las filas iniciales limpias antes de abrirlo
    agregarRed();
    agregarPrecio();

    // 2. SEGUNDO: Inicializamos el modal si es la primera vez
    if (!bootstrapPlanModal) {
        bootstrapPlanModal = new bootstrap.Modal(document.getElementById("modalPlan"));
    }
    
    // 3. TERCERO: Ahora que el contenido es estático, lo mostramos de forma segura
    bootstrapPlanModal.show();
}
...
function resetearFormularioPlan() {
    ...
    if(primeraFila) actualizarEstadoRango(primeraFila);
}

function cerrarModalPlan() {
    if (bootstrapPlanModal) {
        bootstrapPlanModal.hide();
    }
    resetearFormularioPlan();
}
//----------------- CARGAR PLANES ------------//
